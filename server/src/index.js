...
app.get('/health', (_req, res) => {
  res.json({ ok: true, gemini: process.env.GEMINI_API_KEY ? 'configured' : 'missing' });
});
...
app.post('/flyers/extract', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No PDF uploaded — send it as multipart field "file".' });
    }
    if (req.file.mimetype && !req.file.mimetype.includes('pdf')) {
      return res.status(415).json({ error: `Expected a PDF, got "${req.file.mimetype}".` });
    }
    ...
    try {
      ...
      return res.status(422).json({ error: `Could not read the PDF: ${err.message}` });
    }
    ...
    console.log(
      `[extract] ${req.file.originalname} — ${rendered.renderedPages} page(s), ` +
        `${products.length} product(s), ${((Date.now() - t0) / 1000).toFixed(1)}s`,
    );
    ...
    res.json({
      ...
      products,
      usage,
      meta: { totalPages: rendered.totalPages, renderedPages: rendered.renderedPages },
    });
  } catch (err) {
...
app.listen(PORT, () => {
  ...
  if (!process.env.GEMINI_API_KEY) {
    console.warn('  ⚠  GEMINI_API_KEY is not set — copy server/.env.example to server/.env');
